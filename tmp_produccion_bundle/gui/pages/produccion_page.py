...
from PySide6.QtGui import QColor, QDesktopServices, QPainter, QPixmap, QFont
try:
...
class ProduccionPage(QWidget):
    ...
    def _build_tab_personal(self) -> None:
        ...
        for c in (self.kpi_disp, self.kpi_asig, self.kpi_morning, self.kpi_afternoon):
            c.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
            kpis.addWidget(c, 1)
        lay.addLayout(kpis)

        # turnos ya están en la fila de KPIs
        ...
        if QChartView and QPieSeries:
            ...
            self.pie_series.setHoleSize(0.6)
            self.pie_series.setPieSize(0.72)
            ...
            self.pie_view.installEventFilter(self)
    ...
    def _refresh_kpis(self) -> None:
        ...
        self.kpi_disp.layout().itemAt(1).widget().setText(str(sum(1 for t in self.store.techs if t.status == "DISPONIBLE")))
        self.kpi_asig.layout().itemAt(1).widget().setText(str(sum(1 for t in self.store.techs if t.status == "ASIGNADO")))
    ...
    def _refresh_pie(self) -> None:
        ...
        if not counts:
            ...
            self._fit_pie_to_view()
            return
        palette = [
            "#1D4ED8", "#2563EB", "#3B82F6", "#38BDF8", "#22C55E",
            "#F59E0B", "#EF4444", "#8B5CF6", "#0EA5E9", "#14B8A6",
        ]
        font = QFont()
        font.setPointSize(7)
        for idx, (op, cnt) in enumerate(sorted(counts.items())):
            slice_ = self.pie_series.append(op, cnt)
            ...
            if QPieSlice:
                ...
                try:
                    slice_.setLabelArmLengthFactor(0.1)
                except Exception:
                    pass
        self._fit_pie_to_view()
    ...
    def eventFilter(self, source, event) -> bool:
        if source is getattr(self, "pie_view", None) and event.type() == QEvent.Resize:
            self._fit_pie_to_view()
        return super().eventFilter(source, event)

    def _fit_pie_to_view(self) -> None:
        if not getattr(self, "pie_series", None):
            return
        if not getattr(self, "pie_view", None):
            return
        w = self.pie_view.viewport().width()
        h = self.pie_view.viewport().height()
        if w <= 0 or h <= 0:
            return
        pad = 24
        radius = max(0.42, min(0.62, (min(w, h) - pad * 2) / min(w, h)))
        self.pie_series.setPieSize(radius)
