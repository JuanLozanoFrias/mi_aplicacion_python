...
for i,l in enumerate(lines):
    if 'alts = regla.get("alternativas"' in l:
        start = i
        break
for j,l in enumerate(lines):
    if 'debug_rows.append((-1, display_name, "JSON-NO"' in l:
        end = j
        break
                ...
                for chosen in alts_to_apply:
                    ...
                    if not expr:
                        ...
                        if parts:
                            ...
                            if perms:
                                expr += "=" + ",".join(perms)
                    # ensure prefix for execute_command
                                ...
                                if isinstance(step3, dict):
                                    ...
                                    for k, v in step3.items():
                                        ...
                                        if want in kn or kn in want:
                                            try:
                                                s = str(v["value"] if isinstance(v, dict) and "value" in v else v)
...
print('patched', start, end, '->', len(new_lines))
