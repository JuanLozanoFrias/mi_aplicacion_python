...
class Step3OptionsPanel(QWidget):
    ...
    def __init__(self) -> None:
        ...
        host = QFrame()
        host.setStyleSheet("QFrame{background:#2a2d3c;border-radius:12px;}")
        ...
        title.setStyleSheet("color:#e0e0e0;font-weight:700;font-size:16px;")
        host_layout.addWidget(title)

        # Panel con dos columnas
        self._panel = QFrame()
        self._panel.setStyleSheet("QFrame{background:#2f3242;border:1px solid #42465a;border-radius:10px;}")
        self._gridCols = QGridLayout(self._panel)
        self._gridCols.setContentsMargins(12, 12, 12, 12)
        self._gridCols.setHorizontalSpacing(12)
        self._gridCols.setVerticalSpacing(12)
    ...
    def _build_row(self, pregunta: str, opts: List[str], typ: str) -> QWidget:
        row = QFrame()
        row.setStyleSheet("""
            QFrame{ background:#303446; border:1px solid #42465a; border-radius:10px; }
        """)
        h = QHBoxLayout(row); h.setContentsMargins(10, 8, 10, 8); h.setSpacing(12)

        lab = QLabel(pregunta)
        lab.setStyleSheet("color:#e0e0e0; font-weight:600;")
        lab.setMinimumWidth(260); lab.setMaximumWidth(360)
        h.addWidget(lab, 0, Qt.AlignVCenter)

        if typ == "radio":
            rb_si = QRadioButton("Sí"); rb_no = QRadioButton("No")
            for rb in (rb_si, rb_no): rb.setStyleSheet("color:#e0e0e0;")
            grp = QButtonGroup(row); grp.addButton(rb_si); grp.addButton(rb_no)
            wrap = QFrame(); hw = QHBoxLayout(wrap); hw.setContentsMargins(0,0,0,0); hw.setSpacing(16)
            hw.addWidget(rb_si); hw.addWidget(rb_no); hw.addStretch(1)
            h.addWidget(wrap, 1)
            self._inputs[pregunta] = {"type":"radio","rb_si":rb_si,"rb_no":rb_no,"group":grp}
        elif typ == "spin":
            sp = QSpinBox(); sp.setRange(0,5); sp.setFixedWidth(80)
