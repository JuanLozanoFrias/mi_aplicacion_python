...
def load_equipo_options(
    ...
    book = (
        path
        or Path(__file__).resolve().parents[2] / "data" / "basedatos.xlsx"
    )
    if not book.exists():
...
class CargaPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # ===== formulario =====
        form = QFrame()
        form.setStyleSheet("background:#2a2d3c;border-radius:12px;")
        ...
        gen_btn = QPushButton("GENERAR TABLA"); gen_btn.setStyleSheet("""
            QPushButton{background:#5b8bea;color:#fff;font-weight:600;border:none;border-radius:6px;padding:8px 18px;}
