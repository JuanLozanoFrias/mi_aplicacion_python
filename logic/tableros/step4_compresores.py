...
def _norm(s: str) -> str:
    return (s or "").strip().upper().translate(_TRANS)
...
def build_compresores_tables(
    ...
    tables = engine.build(step2_state, globs)
    # Actualizar títulos con separador visual
    for t in tables:
        t.title = f"{t.comp_key} · Arranque: {t.arranque}"
    return tables
