...
class MaterialesCore:
    ...
    def _normalize_action(txt: str) -> str:
        ...
        if "*PONE" in u:
            return "PONE"
        u = u.replace("É", "E").replace("Í", "I").replace("Á", "A").replace("Ó", "O").replace("Ú", "U")
        if u == "GENÉRICO":
            u = "GENERICO"
        return u
    ...
    def _row_matches(
        ...
        for col_letter, selector in conds:
            ...
            if up_sel in ("UL", "IEC"):
                ...
                # 2) Columna con etiquetas UL/IEC/UL-IEC/IEC-UL...
                has_ul  = "UL"  in cell_u
            ...
            # ---------- comodín general SOLO para no-UL/IEC ----------
            if cell_val == "*":
                continue
            ...
            # Comparación de texto exacta (case-insensitive)
            if cell_u == up_sel:
                continue
